defmodule OberonWeb.UserAuth do
  ...
  @max_age 60 * 60 * 24 * 60
  @remember_me_cookie "_oberon_web_user_remember_me"
  ...
  def log_out_user(conn) do
    ...
    if live_socket_id = get_session(conn, :live_socket_id) do
      OberonWeb.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end
  ...
  the `current_scope`:
      ...
      defmodule OberonWeb.PageLive do
        use OberonWeb, :live_view

        on_mount {OberonWeb.UserAuth, :mount_current_scope}
        ...
      end

  Or use the `live_session` of your router to invoke the on_mount callback:

      live_session :authenticated, on_mount: [{OberonWeb.UserAuth, :require_authenticated}] do
  ...
  def disconnect_sessions(tokens) do
    Enum.each(tokens, fn %{token: token} ->
      OberonWeb.Endpoint.broadcast(user_session_topic(token), "disconnect", %{})
    end)
  end
